...
class Evaluator(object):
    ...
    def _sample_molecules(
        ...
        if inpaint_scaffold:
            ...
            print("running scored sampling")
            func = self._generate_molecule_scored
    ...
    @torch.no_grad()
    def _generate_molecule_scored(self, batch):
        batch = batch.to(self.model.device)
        sample_results = self.model.model.sample(batch)
        # final_output = sample_results[-1]
        
        molecules_step = [self.molecule_builder(result) for result in sample_results]
        best_score_mols = self.postprocess_molecules(x_molecule_list= molecules_step)
        # molecules = self.molecule_builder(final_output)
        return best_score_mols
