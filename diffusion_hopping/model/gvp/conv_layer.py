...
class GVPMessagePassing(MessagePassing, ABC):
    ...
    def forward(self, x: s_V, edge_index: torch.Tensor, edge_attr: torch.Tensor) -> s_V:
        s, V = x
        s = s.to(torch.float32)  # Ensure scalar features are in float32
        V = V.to(torch.float32)  # Ensure vector features are in float32
        v_dim = V.shape[-1]
        V = torch.flatten(V, start_dim=-2, end_dim=-1)
        return self.propagate(edge_index, s=s, V=V, edge_attr=edge_attr, v_dim=v_dim)
    ...
    def message(self, s_i, s_j, V_i, V_j, edge_attr, v_dim):
        ...
        V_i = V_i.view(*V_i.shape[:-1], self.in_vector, v_dim).to(torch.float32)
        V_j = V_j.view(*V_j.shape[:-1], self.in_vector, v_dim).to(torch.float32)
        edge_scalar, edge_vector = edge_attr
        edge_scalar = edge_scalar.to(torch.float32)
        edge_vector = edge_vector.to(torch.float32)
        s = torch.cat([s_i, s_j, edge_scalar], dim=-1)
        V = torch.cat([V_i, V_j, edge_vector], dim=-2)
    ...
    def update(self, aggr_out: torch.Tensor) -> s_V:
        ...
        if self.aggr == "add" or self.aggr == "sum":
            s_aggr /= self.normalization_factor
            V_aggr /= self.normalization_factor
        # print("s_aggr after :", s_aggr )
        # print("V_aggr after:", V_aggr )
...
class GVPConvLayer(GVPMessagePassing, ABC):
    ...
    def forward(
        ...
        # print("Before super - Scalars:", x[0][:2], "Vectors:", x[1][:1, :1])
        s, V = super().forward(x, edge_index, edge_attr)
        # print("s_0", s)
        # print("V_0", V)
        # print("After super - Scalars:", s[:2], "Vectors:", V[:1, :1])
        if self.residual:
