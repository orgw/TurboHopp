...
class ProteinLigandDataset(InMemoryDataset):
    def __init__(
        ...
        if split == "train":
            self.data, self.slices = self._load_and_merge_train_val_test()
        else:
            split_file = Path(self.processed_dir) / f"{split}.pt"
            self.data, self.slices = torch.load(split_file)

    def _load_and_merge_train_val_test(self):
        # Load and concatenate train, val, and test datasets
        splits_to_merge = ["train", "val", "test"]
        datasets = []
        for split in splits_to_merge:
            split_file = Path(self.processed_dir) / f"{split}.pt"
            data, slices = torch.load(split_file)
            datasets.append((data, slices))
        
        # Here you would concatenate the datasets. This is a simplified example.
        # You'll need to implement the actual merging logic based on your dataset structure.
        merged_data, merged_slices = self._merge_datasets(datasets)
        return merged_data, merged_slices

    def _merge_datasets(self, datasets):
        # Placeholder for the actual merge logic.
        # You need to merge datasets and slices in a way that makes sense for your data.
        # This could involve concatenating tensors and adjusting slices accordingly.
        return datasets[0]  # Placeholder return

    @property
    def processed_file_names(self) -> Union[str, List[str], Tuple]:
        if self.split == "train":
            # Return all file names for train, val, and test, since we're merging them
            return ["train.pt", "val.pt", "test.pt"]
        else:
            # For predict or any other split, just return the respective file
